import { useRespostasReclamacao } from '@/hooks/useReclamacoes';
...
export default function RespostasReclamacaoScreen() {
  const insets = useSafeAreaInsets();
  const { id } = useLocalSearchParams();
  const fadeAnim = useRef(new Animated.Value(0)).current;
  const slideAnim = useRef(new Animated.Value(50)).current;

  const { data: respostas = [], isLoading, error, refetch } = useRespostasReclamacao(Number(id));
      ...
      <View style={styles.header}>
        <View style={styles.headerTop}>
          ...
          <View style={styles.titleContainer}>
            <Text style={styles.headerTitle}>Respostas da Reclamação</Text>
          </View>
