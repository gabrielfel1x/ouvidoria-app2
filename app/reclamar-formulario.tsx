...
export default function ReclamarFormularioScreen() {
  ...
  const handleLoadCurrentLocation = async () => {
    ...
    try {
      ...
      // Tenta fazer geocodificação reversa para obter endereço legível
      try {
        ...
        // Se falhar, usa apenas as coordenadas
        setAddress(`${latitude.toFixed(6)}, ${longitude.toFixed(6)}`);
      }
      ...
      toast.success(
        'Sucesso!', 
        {
          description: `Localização obtida com precisão de ${accuracy ? Math.round(accuracy) : '?'}m`,
        }
      );
    } catch (error) {
      toast.error('Erro', {
  ...
  const handleAddressSearch = async (searchText: string) => {
    ...
    try {
      // Usa geocodificação para buscar endereços
      const geocodeResults = await Location.geocodeAsync(searchText);
      
      if (geocodeResults && geocodeResults.length > 0) {
        // Converte os resultados em sugestões legíveis
        const suggestions = await Promise.all(
          geocodeResults.slice(0, 5).map(async (result, index) => {
            try {
              // Faz geocodificação reversa para obter endereço legível
              const reverseGeocode = await Location.reverseGeocodeAsync({
                latitude: result.latitude,
                longitude: result.longitude,
              });
              
              let formattedAddress = searchText; // Fallback para o texto pesquisado
              ...
              // Se falhar a geocodificação reversa, usa o texto pesquisado
              return {
                id: index,
                latitude: result.latitude,
                longitude: result.longitude,
